

/**
 * Created by ronik.basak on 01/09/16.
 */
public class MatrixProbability {
    public boolean isSafe(int x, int y, int m, int n){
        return(x>=0 && x<m && y>=0 && y<n);
    }

    // Function to calculate probability that after N
    // moves from a given position (x, y) in m x n matrix,
    // boundaries of the matrix will not be crossed.

    public double findProbability(int m, int n, int x, int y, int N){
        //boundary crossed
        if(!isSafe(x,y,m,n))
            return 0.0;

        // N steps taken
        if(N == 0)
            return 1.0;

        //Initialize result
        double prob = 0.0;

        //move left
        prob += findProbability(m,n,x-1,y,N-1)*0.25;

        //move up
        prob += findProbability(m,n,x,y+1,N-1)*0.25;

        //move down
        prob += findProbability(m,n,x,y-1,N-1)*0.25;

        //move right
        prob += findProbability(m,n,x+1,y,N-1)*0.25;

        return prob;
    }

    public static void main(String[] args) {
        int m=5, n=5;
        int i=1, j=1;
        int N=2;
        MatrixProbability mat = new MatrixProbability();
        System.out.println("Probability is = "+ mat.findProbability(m,n,i,j,N));
    }
}
